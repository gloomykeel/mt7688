...
#else
//alex.kou del config 092019
		printf("Erase len  :0x%08x\n", CFG_BOOTLOADER_SIZE);
		printf("Erase end  :0x%08x\n",0+0xbc000000 + CFG_BOOTLOADER_SIZE);
#endif
		printf("**************************\n");
...
int do_run(cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	...
	if (!strncmp(argv[1], "lu", 2)) {
		 ...
		 */
		if (run_command("tftpboot 0x80100000 uboot.bin", 0)) {
		...
		if (run_command("tftpboot 0x80100000 firmware.bin", 0)) {
			if (!TftpdownloadStatus) {
...
U_BOOT_CMD(
	...
	"run	- tftp-erase-cp \n",
	"\nrun lu:\n"
	"    - download uboot.bin from your tftp server and replace the current one\n\n"
	"run lf:\n"
	"    - download firmware.bin	from your tftp server and write on flash\n\n"
	"run lc:\n"
